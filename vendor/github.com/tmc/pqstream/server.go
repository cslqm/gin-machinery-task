...
func (s *Server) InstallTriggersByTableName(tableNames []string) error {
	_, err := s.db.Exec(sqlTriggerFunction)
	if err != nil {
		return err
	}
...
func (s *Server) ThorHandleEvents(eventchan chan *pq.Notification) error {
	subscribers := map[*subscription]bool{}
	events := s.l.NotificationChannel()
	for {
		select {
